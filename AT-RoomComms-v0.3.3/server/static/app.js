...
async function load(){try{S.d=await api('/api/bootstrap');showOnly('app');if(S.d.me.kind==='operator'){await renderOperatorApp()}else{renderChrome();control()}wsConnect();S.sendDeviceHeartbeat?.();return true}catch(e){console.error(e);showOnly('login');const box=$('#loginError');if(box)box.textContent='RoomComms could not load: '+e.message;return false}}
