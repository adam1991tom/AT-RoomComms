...
async function eventReport(id){
    ...
    $('#content').innerHTML=`<div class="toolbar"><button class="btn secondary" onclick="openEvent(${id})">← Back to event</button><button class="btn" onclick="downloadEventReportCsv()">⬇ Download CSV</button></div><div class="card body"><h2>${esc(r.event.name)}</h2><p class="operator">${esc(r.event.client||'No client')} · ${esc(r.event.event_status)} · ${esc(r.event.starts_at||'')} → ${esc(r.event.ends_at||'')}</p><p><b>${r.rooms.length}</b> rooms &nbsp; <b>${totalMsgs}</b> messages &nbsp; <b>${r.help_requests.length}</b> help requests &nbsp; <b>${r.emergencies.length}</b> emergency alerts</p></div><div class="card"><div class="cardHead"><h2>Rooms</h2></div>${roomRows}</div><div class="card"><div class="cardHead"><h2>Help Requests</h2></div>${helpRows}</div><div class="card"><div class="cardHead"><h2>Emergency Alerts</h2></div>${emgRows}</div>`;
}
