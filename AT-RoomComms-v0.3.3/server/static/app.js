...
function hexToRgbStr(hex){const n=hex.replace('#','');const r=parseInt(n.substr(0,2),16),g=parseInt(n.substr(2,2),16),b=parseInt(n.substr(4,2),16);return `${r},${g},${b}`}
function shadeHex(hex,pct){const n=hex.replace('#','');let r=parseInt(n.substr(0,2),16),g=parseInt(n.substr(2,2),16),b=parseInt(n.substr(4,2),16);const f=(c)=>Math.max(0,Math.min(255,Math.round(c+(pct<0?c:255-c)*pct)));return '#'+[f(r),f(g),f(b)].map(x=>x.toString(16).padStart(2,'0')).join('')}
function resolveAccent(){return localStorage.getItem('rc_pref_accent')||S.d?.settings?.ui_accent||''}
function applyTheme(theme,accent){
    ...
    if(accent&&/^#[0-9a-f]{6}$/i.test(accent)){
        ...
        root.setProperty('--accent2',shadeHex(accent,-0.35));
        root.setProperty('--accent-rgb',hexToRgbStr(accent));
    }else{
        root.removeProperty('--accent');root.removeProperty('--accent2');root.removeProperty('--accent-rgb');
    }
}
function statusLabel(s){
    let custom={};try{custom=JSON.parse(S.d?.settings?.status_labels||'{}')}catch{}
    return custom[s]||({closed:'Closed',setting_up:'Setting Up',ready:'Ready',rehearsal:'Rehearsal',live:'Live',technical_issue:'Technical Issue',help_requested:'Help Requested'}[s]||s);
}
function priLabel(p){
    let custom={};try{custom=JSON.parse(S.d?.settings?.priority_labels||'{}')}catch{}
    return custom[p]||({normal:'Normal',important:'Important',urgent:'Urgent',emergency:'Emergency'}[p]||p);
}
...
function prefSoundOn(){return localStorage.getItem('rc_pref_sound')!=='0'}
function prefDesktopOn(){return localStorage.getItem('rc_pref_desktop')==='1'}
function prefFontSize(){return localStorage.getItem('rc_pref_fontsize')||'md'}
function applyFontSizePref(){const el=document.getElementById('app');if(!el)return;el.style.zoom=({sm:'0.9',md:'1',lg:'1.15'})[prefFontSize()]||'1'}
let _audioCtx=null;
function playBeep(priority){
    try{
        _audioCtx=_audioCtx||new (window.AudioContext||window.webkitAudioContext)();
        const beep=(freq,delay,dur)=>{const o=_audioCtx.createOscillator(),g=_audioCtx.createGain();o.frequency.value=freq;o.connect(g);g.connect(_audioCtx.destination);g.gain.setValueAtTime(0.15,_audioCtx.currentTime+delay);g.gain.exponentialRampToValueAtTime(0.001,_audioCtx.currentTime+delay+dur);o.start(_audioCtx.currentTime+delay);o.stop(_audioCtx.currentTime+delay+dur)};
        if(priority==='emergency'){beep(880,0,.18);beep(660,.22,.18);beep(880,.44,.22)}
        else if(priority==='urgent'){beep(760,0,.15);beep(760,.18,.15)}
        else beep(700,0,.14);
    }catch{}
}
function notifyNative(title,body,priority){
    ...
    if(prefSoundOn())playBeep(priority);
    if(prefDesktopOn()&&'Notification' in window&&Notification.permission==='granted'){
...
async function load(){try{
    S.d=await api('/api/bootstrap');showOnly('app');applyFontSizePref();
...
async function renderOperatorApp(){
    ...
    $('#eventNav').innerHTML='';
    $('#userBox').innerHTML=`<b>${esc(d.me.display_name)}</b><br>${d.me.device_role==='main'?'Main PC':'Backup PC'}${d.me.device_role==='main'?' · notifications off':''}<br><button class="linkBtn" onclick="openPreferences()">Preferences</button>`;
    ...
    $('#content').innerHTML=`<div id="opHelpBanner"></div><div class="card feedCard"><div class="cardHead"><div><h2>Operations Feed</h2><small>${esc(d.event?.name||'')} · ${statusLabel(d.room?.current_status||'closed')}</small></div><div class="headActions"><button class="btn danger" onclick="emergencyAlert(${d.event?.id||0},${d.room?.id||0})">🚨 Emergency</button><button class="btn danger" onclick="helpRequest(${d.event?.id||0},${d.room?.id||0})">Request Help</button></div></div><div class="feed" id="feedList">${ms.map(messageHtml).join('')||'<div class="body feedEmpty">No messages yet.</div>'}</div><div class="composer"><div style="display:flex;gap:6px">${attachBtn('file')}${emojiBtn('msgText')}</div><input class="inlineInput" id="msgText" placeholder="Type an operational message" onkeydown="if(event.key==='Enter'){event.preventDefault();sendOperatorRoom()}"><select id="msgPri" class="statusSelect priSelect">${['normal','important','urgent','emergency'].map(p=>`<option value="${p}">${esc(priLabel(p))}</option>`).join('')}</select><button class="btn" onclick="sendOperatorRoom()">Send</button></div></div>`;
    renderOpHelpBanner();
...
function title(t,s=''){$('#title').textContent=t;$('#sub').textContent=s;if(S.d?.me?.kind==='account')renderChrome();closeMobileNav()}
function openMobileNav(){$('aside')?.classList.add('open');$('#navOverlay')?.classList.add('show')}
function closeMobileNav(){$('aside')?.classList.remove('open');$('#navOverlay')?.classList.remove('show')}
...
const STATUS_KEYS=[['closed','Closed'],['setting_up','Setting Up'],['ready','Ready'],['rehearsal','Rehearsal'],['live','Live'],['technical_issue','Technical Issue']];
const PRI_KEYS=[['normal','Normal'],['important','Important'],['urgent','Urgent'],['emergency','Emergency']];
function settings(){
    if(S.d.me.role!=='admin')return;S.view='settings';S.rerender=settings;title('System Settings','Administrator only');
    const curTheme=S.d.settings.ui_theme||'blue';const curAccent=S.d.settings.ui_accent||'';
    const themes=[['blue','Blue'],['purple','Purple'],['green','Green'],['orange','Orange']];
    let statusLabels={};try{statusLabels=JSON.parse(S.d.settings.status_labels||'{}')}catch{}
    let priorityLabels={};try{priorityLabels=JSON.parse(S.d.settings.priority_labels||'{}')}catch{}
    S.pendingTheme=curTheme;S.pendingAccent=curAccent;
    $('#content').innerHTML=`<div class="card body"><div class="form">
<label>Venue name<input id="setVenue" value="${esc(S.d.settings.venue_name||'')}"></label>
<label>Control Centre display name<input id="setControl" value="${esc(S.d.settings.control_centre_name||'')}"></label>
<label>Daily sign-out time (UTC, everyone logged off)<input id="setLogoff" placeholder="03:00" value="${esc(S.d.settings.daily_logoff_utc||'03:00')}"></label>
<label>Attachment size limit (MB)<input id="setAttachLimit" type="number" min="1" max="500" value="${esc(S.d.settings.attachment_limit_mb||'25')}"></label>
<label>Feed history length (messages kept per feed)<input id="setFeedLimit" type="number" min="20" max="2000" value="${esc(S.d.settings.feed_limit||'250')}"></label>
<label>Colour theme<div class="themeSwatches">${themes.map(([id,label])=>`<button type="button" class="themeSwatch ${id===curTheme&&!curAccent?'active':''}" data-theme-id="${id}" title="${label}" onclick="pickTheme('${id}')"></button>`).join('')}</div></label>
<label>Custom accent colour (overrides the swatch above)<div style="display:flex;gap:10px;align-items:center"><input type="color" id="setAccent" value="${curAccent||'#1ea0dc'}" oninput="previewAccent(this.value)"><button type="button" class="btn small secondary" onclick="clearAccent()">Use preset colour</button></div></label>
<h3>Room status labels</h3>
${STATUS_KEYS.map(([k,def])=>`<label>${esc(def)}<input class="setStatusLabel" data-key="${k}" value="${esc(statusLabels[k]||def)}"></label>`).join('')}
<h3>Priority labels</h3>
${PRI_KEYS.map(([k,def])=>`<label>${esc(def)}<input class="setPriLabel" data-key="${k}" value="${esc(priorityLabels[k]||def)}"></label>`).join('')}
<h3>Help request categories</h3>
<label>Comma separated<input id="setHelpCats" value="${esc(S.d.settings.help_categories||'Presentation,Video,Audio,Lighting,Network,Room Setup,Speaker Support,Other')}"></label>
<button class="btn" id="saveSettings">Save</button>
</div></div>`;
    $('#saveSettings').onclick=async()=>{
        const statusLbls={};$$('.setStatusLabel').forEach(i=>{if(i.value.trim())statusLbls[i.dataset.key]=i.value.trim()});
        const priLbls={};$$('.setPriLabel').forEach(i=>{if(i.value.trim())priLbls[i.dataset.key]=i.value.trim()});
        await api('/api/settings',{method:'PATCH',body:JSON.stringify({
            venue_name:$('#setVenue').value,
            control_centre_name:$('#setControl').value,
            daily_logoff_utc:$('#setLogoff').value.trim(),
            attachment_limit_mb:$('#setAttachLimit').value,
            feed_limit:$('#setFeedLimit').value,
            ui_theme:S.pendingTheme||curTheme,
            ui_accent:S.pendingAccent||'',
            status_labels:JSON.stringify(statusLbls),
            priority_labels:JSON.stringify(priLbls),
            help_categories:$('#setHelpCats').value.trim()
        })});
        await refresh();settings();
    }
}
window.settings=settings;
function previewAccent(hex){S.pendingAccent=hex;$$('.themeSwatch').forEach(b=>b.classList.remove('active'));applyTheme(S.pendingTheme||S.d.settings.ui_theme,hex)}
window.previewAccent=previewAccent;
function clearAccent(){S.pendingAccent='';$('#setAccent').value='#1ea0dc';const cur=S.pendingTheme||S.d.settings.ui_theme||'blue';$$('.themeSwatch').forEach(b=>b.classList.toggle('active',b.dataset.themeId===cur));applyTheme(cur,'')}
window.clearAccent=clearAccent;
function pickTheme(id){S.pendingTheme=id;S.pendingAccent='';applyTheme(id,'');$$('.themeSwatch').forEach(b=>b.classList.toggle('active',b.dataset.themeId===id))}
window.pickTheme=pickTheme;

function openPreferences(){
    const myAccent=localStorage.getItem('rc_pref_accent')||'';
    let prefCleared=!myAccent;
    modal(`<h2>My Preferences</h2><p class="notice">These only affect how RoomComms looks and sounds on this device.</p><div class="form">
<label>My accent colour<div style="display:flex;gap:10px;align-items:center"><input type="color" id="prefAccent" value="${myAccent||S.d.settings.ui_accent||'#1ea0dc'}" oninput="window._prefAccentChanged(this.value)"><button type="button" class="btn small secondary" id="prefAccentClear">Use venue default</button></div></label>
<label>Text size<select id="prefFontSize"><option value="sm">Small</option><option value="md">Medium</option><option value="lg">Large</option></select></label>
<label class="checkboxLabel"><input type="checkbox" id="prefSound"><span>Play a sound for new message notifications<small>Applies in the browser (the Windows client has its own notification sound).</small></span></label>
<label class="checkboxLabel"><input type="checkbox" id="prefDesktop"><span>Show desktop notifications<small>Your browser will ask for permission the first time you turn this on.</small></span></label>
<div><button class="btn" id="prefSave">Save</button> <button class="btn secondary" onclick="closeModal()">Cancel</button></div>
</div>`);
    $('#prefFontSize').value=prefFontSize();
    $('#prefSound').checked=prefSoundOn();
    $('#prefDesktop').checked=prefDesktopOn();
    window._prefAccentChanged=(hex)=>{prefCleared=false;applyTheme(S.d.settings.ui_theme,hex)};
    $('#prefAccentClear').onclick=()=>{prefCleared=true;$('#prefAccent').value=S.d.settings.ui_accent||'#1ea0dc';applyTheme(S.d.settings.ui_theme,S.d.settings.ui_accent||'')};
    $('#prefSave').onclick=async()=>{
        const wantDesktop=$('#prefDesktop').checked;
        if(wantDesktop&&'Notification' in window&&Notification.permission==='default'){
            try{await Notification.requestPermission()}catch{}
        }
        const desktopGranted=wantDesktop&&'Notification' in window&&Notification.permission==='granted';
        if(prefCleared)localStorage.removeItem('rc_pref_accent');else localStorage.setItem('rc_pref_accent',$('#prefAccent').value);
        localStorage.setItem('rc_pref_fontsize',$('#prefFontSize').value);
        localStorage.setItem('rc_pref_sound',$('#prefSound').checked?'1':'0');
        localStorage.setItem('rc_pref_desktop',desktopGranted?'1':'0');
        applyFontSizePref();
        applyTheme(S.d.settings.ui_theme,resolveAccent());
        closeModal();
    };
}
window.openPreferences=openPreferences;
async function accounts(){if(S.d.me.role!=='admin')return;S.view='accounts';S.rerender=accounts;title('Login Accounts','Administrator only');const a=await api('/api/accounts');$('#content').innerHTML=`<div class="toolbar"><button class="btn" onclick="newAccount()">+ Login Account</button></div><div class="card">${a.map(x=>`<div class="listRow"><b>${esc(x.display_name)}</b><small>${esc(x.username)} · ${x.role==='admin'?'Administrator':'Speaker Preview'}</small><button class="btn small secondary" onclick="resetPassword(${x.id},'${esc(jsStr(x.username))}')">Password</button>${x.id!==S.d.me.id?`<button class="btn small danger" onclick="disableAccount(${x.id})">Disable</button>`:''}</div>`).join('')}</div>`}window.accounts=accounts;function newAccount(){modal(`<h2>New Login Account</h2><div class="form"><label>Username<input id="acUser"></label><label>Display name<input id="acName"></label><label>Password<input id="acPass" type="password"></label><label>Role<select id="acRole"><option value="speaker_preview">Speaker Preview</option><option value="admin">Administrator</option></select></label><button class="btn" id="saveAccount">Create</button></div>`);$('#saveAccount').onclick=async()=>{try{await api('/api/accounts',{method:'POST',body:JSON.stringify({username:$('#acUser').value,display_name:$('#acName').value,password:$('#acPass').value,role:$('#acRole').value})});closeModal();accounts()}catch(e){alert(e.message)}}}window.newAccount=newAccount;async function resetPassword(id,u){const p=prompt(`New password for ${u}:`);if(!p)return;await api(`/api/accounts/${id}/password`,{method:'PATCH',body:JSON.stringify({password:p})});alert('Password changed.')}window.resetPassword=resetPassword;async function disableAccount(id){if(confirm('Disable this login?')){await api(`/api/accounts/${id}`,{method:'DELETE'});accounts()}}window.disableAccount=disableAccount;
async function venue(){
    ...
    $('#content').innerHTML=`<div class="card"><div class="cardHead"><h2>Venue Overview</h2><div>${statPills}</div></div>${roomGrid}</div><div class="card feedCard"><div class="cardHead"><h2>Broadcast Feed</h2></div><div class="feed" id="feedList">${ms.map(messageHtml).join('')||'<div class="body feedEmpty">No messages yet.</div>'}</div><div class="composer"><div style="display:flex;gap:6px">${attachBtn('venueFile')}${emojiBtn('venueMsg')}</div><input id="venueMsg" class="inlineInput" placeholder="Venue message" onkeydown="if(event.key==='Enter'){event.preventDefault();sendVenue()}"><select id="venuePri" class="statusSelect priSelect">${['normal','important','urgent'].map(p=>`<option value="${p}">${esc(priLabel(p))}</option>`).join('')}</select><button class="btn" onclick="sendVenue()">Send</button></div></div>`;scrollFeedBottom();
}
...
function devicesPage(){
    ...
    $('#content').innerHTML=`<div class="card">${list.map(d=>{
        ...
        return `<div class="listRow"><span class="dot ${stale?'':'ready'}"></span><b>${esc(d.name)}</b><small>${esc(d.role||'')}${d.operator?` · ${esc(d.operator)}`:''}</small>${deviceRoomLabel(d)}${d.presenting?'<span class="badge">🖥️ Presenting</span>':''}<small>v${esc(d.app_version||'?')}</small><small>Uptime ${fmtDuration(d.uptime_seconds)}</small><small>${fmtAgo(d.last_heartbeat)}</small>${diag&&Object.keys(diag).length?`<button class="btn small secondary" data-diag-device-id="${d.id}">Diagnostics</button>`:''}</div>`;
    }).join('')||'<div class="body">No devices have reported in yet.</div>'}</div>`;
    $('#content').querySelectorAll('[data-diag-device-id]').forEach(btn=>btn.addEventListener('click',()=>showDeviceDiagnostics(Number(btn.dataset.diagDeviceId))));
}
window.devicesPage=devicesPage;
function fmtDurationMs(ms){if(isNaN(ms)||ms<0)return'—';return fmtDuration(Math.floor(ms/1000))}
function csvEsc(v){return '"'+String(v??'').replace(/"/g,'""')+'"'}
async function eventReport(id){
    S.view='report';S.rerender=()=>eventReport(id);
    const e=S.d.events.find(x=>x.id===id);
    title(`${e?.name||'Event'} — Report`,'Summary for this event');
    $('#content').innerHTML='<div class="card body">Loading…</div>';
    const r=await api(`/api/events/${id}/report`);
    if(S.view!=='report')return;
    S.reportData=r;
    const totalMsgs=r.rooms.reduce((s,x)=>s+x.message_count,0);
    const roomRows=r.rooms.map(x=>`<div class="listRow"><b>${esc(x.name)}</b><small>${x.operator_name?`👤 ${esc(x.operator_name)}`:'Unassigned'}</small><small>${x.message_count} messages</small><small>${x.first_activity?`${new Date(x.first_activity).toLocaleString()} → ${new Date(x.last_activity).toLocaleString()}`:'No activity'}</small></div>`).join('')||'<div class="body">No rooms in this event.</div>';
    const helpRows=r.help_requests.map(h=>{
        const resolved=h.resolved_at?fmtDurationMs(new Date(h.resolved_at)-new Date(h.created_at)):'Not resolved';
        return `<div class="listRow"><div style="flex:1"><b>${esc(h.room_name||'Room')} · ${esc(h.category)}</b><br><small>${esc(h.description)}</small></div><span class="badge">${esc(h.status)}</span><small>${resolved}</small></div>`;
    }).join('')||'<div class="body">No help requests were raised.</div>';
    const emgRows=r.emergencies.map(m=>`<div class="listRow"><div style="flex:1"><b>${esc(m.sender)}</b><br><small>${esc(m.body)}</small></div><small>${new Date(m.created_at).toLocaleString()}</small></div>`).join('')||'<div class="body">No emergency alerts were raised.</div>';
    $('#content').innerHTML=`<div class="toolbar"><button class="btn secondary" onclick="openEvent(${id})">← Back to event</button><button class="btn" onclick="downloadEventReportPdf(${id})">⬇ Download PDF</button><button class="btn secondary" onclick="downloadEventReportCsv()">⬇ Download CSV</button></div><div class="card body"><h2>${esc(r.event.name)}</h2><p class="operator">${esc(r.event.client||'No client')} · ${esc(r.event.event_status)} · ${esc(r.event.starts_at||'')} → ${esc(r.event.ends_at||'')}</p><p><b>${r.rooms.length}</b> rooms &nbsp; <b>${totalMsgs}</b> messages &nbsp; <b>${r.help_requests.length}</b> help requests &nbsp; <b>${r.emergencies.length}</b> emergency alerts</p></div><div class="card"><div class="cardHead"><h2>Rooms</h2></div>${roomRows}</div><div class="card"><div class="cardHead"><h2>Help Requests</h2></div>${helpRows}</div><div class="card"><div class="cardHead"><h2>Emergency Alerts</h2></div>${emgRows}</div>`;
}
window.eventReport=eventReport;
function downloadEventReportCsv(){
    const r=S.reportData;if(!r)return;
    const lines=['Section,Room,Category/Sender,Description/Body,Status,Created,Resolved'];
    r.help_requests.forEach(h=>lines.push(['Help',h.room_name||'',h.category,h.description,h.status,h.created_at,h.resolved_at||''].map(csvEsc).join(',')));
    r.emergencies.forEach(m=>lines.push(['Emergency','',m.sender,m.body,'',m.created_at,''].map(csvEsc).join(',')));
    const blob=new Blob([lines.join('\n')],{type:'text/csv'});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a');a.href=url;a.download=`${(r.event.name||'event').replace(/[^a-z0-9]+/gi,'_')}_report.csv`;document.body.appendChild(a);a.click();a.remove();
    setTimeout(()=>URL.revokeObjectURL(url),1000);
}
window.downloadEventReportCsv=downloadEventReportCsv;
async function downloadEventReportPdf(id){
    const btn=event?.target;if(btn)btn.disabled=true;
    try{
        const resp=await fetch(`/api/events/${id}/report.pdf`,{headers:{Authorization:'Bearer '+token}});
        if(!resp.ok)throw Error('Failed to generate PDF ('+resp.status+')');
        const blob=await resp.blob();
        const url=URL.createObjectURL(blob);
        const a=document.createElement('a');a.href=url;a.download=`${(S.reportData?.event?.name||'event').replace(/[^a-z0-9]+/gi,'_')}_report.pdf`;document.body.appendChild(a);a.click();a.remove();
        setTimeout(()=>URL.revokeObjectURL(url),1000);
    }catch(err){alert(err.message)}finally{if(btn)btn.disabled=false}
}
window.downloadEventReportPdf=downloadEventReportPdf;
function showDeviceDiagnostics(deviceId){
    const d=(S.d.devices||[]).find(x=>x.id===deviceId);
...
$$('.nav[data-view]').forEach(b=>b.onclick=()=>{const v=b.dataset.view;$$('.nav').forEach(x=>x.classList.remove('active'));b.classList.add('active');({control,venue,manage,operators,help:helpPage,dm:dmPage,settings,accounts,room:renderOperatorApp,issues:issuesPage,devices:devicesPage}[v]||control)()});$('#modal').onclick=e=>{if(e.target===$('#modal'))closeModal()};$('#menuBtn')?.addEventListener('click',openMobileNav);$('#navOverlay')?.addEventListener('click',closeMobileNav);start();