...
CREATE TABLE IF NOT EXISTS rooms(id INTEGER PRIMARY KEY,name TEXT UNIQUE,short_name TEXT,current_status TEXT DEFAULT 'closed',enabled INTEGER DEFAULT 1);
CREATE TABLE IF NOT EXISTS events(id INTEGER PRIMARY KEY,name TEXT,client TEXT,event_color TEXT,starts_at TEXT,ends_at TEXT,event_status TEXT DEFAULT 'scheduled',archived INTEGER DEFAULT 0);
CREATE TABLE IF NOT EXISTS event_rooms(event_id INTEGER,room_id INTEGER,PRIMARY KEY(event_id,room_id));
CREATE TABLE IF NOT EXISTS devices(id INTEGER PRIMARY KEY,name TEXT UNIQUE,role TEXT,room_id INTEGER,event_id INTEGER,operator TEXT,online_status TEXT,last_heartbeat TEXT,app_version TEXT);
CREATE TABLE IF NOT EXISTS messages(id INTEGER PRIMARY KEY,scope TEXT,scope_id INTEGER,sender TEXT,body TEXT,priority TEXT,created_at TEXT);
CREATE TABLE IF NOT EXISTS attachments(id INTEGER PRIMARY KEY,message_id INTEGER,original_name TEXT,stored_name TEXT,mime_type TEXT,size INTEGER);
CREATE TABLE IF NOT EXISTS help_requests(id INTEGER PRIMARY KEY,event_id INTEGER,room_id INTEGER,room_name TEXT,requested_by TEXT,category TEXT,description TEXT,priority TEXT,status TEXT,assigned_to TEXT,created_at TEXT,acknowledged_at TEXT,resolved_at TEXT);
''')
  ...
  if c.execute('SELECT COUNT(*) n FROM rooms').fetchone()['n']==0:
   ...
   e1=c.execute("INSERT INTO events(name,client,event_color,starts_at,ends_at,event_status) VALUES(?,?,?,?,?,'live')",('NHS Conference','NHS England','#8b5cf6','2026-08-09T08:00','2026-08-09T17:00')).lastrowid
   e2=c.execute("INSERT INTO events(name,client,event_color,starts_at,ends_at,event_status) VALUES(?,?,?,?,?,'live')",('Bird Association Meeting','RSPB','#22a7f0','2026-08-09T09:00','2026-08-09T16:00')).lastrowid
   ...
   q3=c.execute("SELECT id FROM rooms WHERE short_name='QS3'").fetchone()['id']; c.execute('INSERT INTO help_requests(event_id,room_id,room_name,requested_by,category,description,priority,status,created_at) VALUES(?,?,?,?,?,?,?,?,?)',(e1,q3,"Queen's Suite 3",'Adam Tomlinson','Presentation',"Presenter's laptop is not displaying through HDMI.",'urgent','new',now()))
...
def messages(scope:str,scope_id:int|None=None):
 with db() as c:
  ...
  for r in reversed(rows.fetchall()):
   d=dict(r); d['attachments']=[dict(a) for a in c.execute('SELECT * FROM attachments WHERE message_id=?',(r['id'],))]; out.append(d)
  return out
@app.post('/api/messages')
async def add_message(m:Message):
 with db() as c:
  mid=c.execute('INSERT INTO messages(scope,scope_id,sender,body,priority,created_at) VALUES(?,?,?,?,?,?,?)',(m.scope,m.scope_id,m.sender,m.body,m.priority,now())).lastrowid; d=dict(c.execute('SELECT * FROM messages WHERE id=?',(mid,)).fetchone()); d['attachments']=[]
...
async def add_help(h:Help):
 with db() as c:
  rn='';
  if h.room_id:
   rr=c.execute('SELECT name FROM rooms WHERE id=?',(h.room_id,)).fetchone(); rn=rr['name'] if rr else ''
